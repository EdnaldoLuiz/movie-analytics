...
import static com.ednaldoluiz.moviedash.constant.CacheConstants.Genre.*;
...
public class GenreService extends AbstractService {
    ...
    @Cacheable(cacheNames = GENRE_CACHE_COUNT + "count", key = "{#genreId}")
    ...
    @Cacheable(cacheNames = GENRE_CACHE_TOTAL, key = "{'total'}")
    ...
    @Cacheable(cacheNames = GENRE_CACHE_VOTE_AVERAGE, key = "{'averageVotes'}")
    ...
    @Cacheable(cacheNames = GENRE_CACHE_POPULARITY, key = "{'popularity'}")
    public List<GenreProjection> getMostPopularGenres() {
        return genreRepository.findMostPopularGenres();
    }

    @Cacheable(cacheNames = GENRE_CACHE_POPULAR_MOVIES, key = "{#genre}")
